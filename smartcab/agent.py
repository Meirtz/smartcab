...
class LearningAgent(Agent):
    ...
    def createQ(self, state):
        ...
        is_state_in_Q = False
        if self.learning:
            for key , _ in self.Q.iteritems():
                if key == state:
                    is_state_in_Q = True
            if is_state_in_Q == False:
                #self.Q.update({state:{}})
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            #valid_actions_withoutNone = (action if action is not 'None' else '' for action in self.valid_actions)
...
def run():
    ...
    sim.run(n_test=20, tolerance=0.0002)
